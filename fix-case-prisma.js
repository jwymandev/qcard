...
// Special relation fields to be handled separately (and remain PascalCase)
...
function fixFile(filePath) {
  ...
  // 1. RELATION FIELDS IN INCLUDE: Should be PascalCase
  ...
  // 2. PROPERTY REFERENCES: Should be PascalCase
  ...
  // 3. CONNECTION REFERENCES: Should be PascalCase
  ...
  // 4. PRISMA CLIENT REFERENCES: Should be lowercase
  // Fix prisma.User -> prisma.user
  for (const model of models) {
    const lowercaseModel = model.charAt(0).toLowerCase() + model.slice(1);
    content = content.replace(new RegExp(`prisma\\.${model}\\b`, 'g'), `prisma.${lowercaseModel}`);
    modified = true;
  }
  
  // 5. SPECIAL RELATION FIELD CASES
  ...
  // 6. SESSION USER REFERENCE FIXES: Should be user not User
  content = content.replace(/session\?\.User\?\.id/g, 'session?.user?.id');
  content = content.replace(/session\.User\.id/g, 'session.user.id');
  modified = true;
