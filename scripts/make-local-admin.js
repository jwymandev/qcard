...
/**
 * Script to promote a user to SUPER_ADMIN role for local development
 * 
 * Usage:
 *   node scripts/make-local-admin.js <user-email>
 * 
 * Example:
 *   node scripts/make-local-admin.js admin@example.com
 */
...
async function makeUserSuperAdmin(email) {
  if (!email) {
    ...
    console.error('Usage: node scripts/make-local-admin.js <user-email>');
    process.exit(1);
  }
