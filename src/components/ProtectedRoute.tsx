...
/**
 * ProtectedRoute component to be used on pages that require authentication
 * This should be used on individual protected pages, not in the main layout
 */
export default function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Check for bypass auth param or cookie
    const urlParams = new URLSearchParams(window.location.search);
    const hasBypassParam = urlParams.has('bypass_auth') || document.cookie.includes('bypass_auth=true');
    
    // If bypass is enabled, skip auth check
    if (hasBypassParam) {
      setIsLoading(false);
      return;
    }
    
    // Set a timer to end loading state after session is checked
    ...
    if (isLoading || status === 'loading') {
      const timer = setInterval(() => {
        setLoadingTime(prev => {
          ...
          // Show error after 3 seconds of loading
          if (newTime === 3) {
            setShowError(true);
          }
          return newTime;
  ...
  // Regular loading screen, will show for up to 3 seconds before the error appears
  if (isLoading || status === 'loading') {
    return (
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          {loadingTime > 2 && (
            <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-md">
              ...
              <Link 
                href={`${currentPath}?bypass_auth=true`}
