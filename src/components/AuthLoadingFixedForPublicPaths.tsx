...
// List of public paths that should never block on authentication
const PUBLIC_PATHS = [
  '/',
  '/sign-in',
  '/sign-up',
...
export default function AuthLoadingFixedForPublicPaths({ children }: { children: React.ReactNode }) {
  ...
  const [isPublicPath, setIsPublicPath] = useState(true); // Default to true for SSR
  ...
  useEffect(() => {
    ...
    // Special case: check for bypass auth param or cookie
    const urlParams = new URLSearchParams(window.location.search);
    const hasBypassParam = urlParams.has('bypass_auth') || document.cookie.includes('bypass_auth=true');
    
    // Set public path status - paths with bypass are treated as public
    setIsPublicPath(pathIsPublic || hasBypassParam);
    
    // If public path, immediately set loading to false
    if (pathIsPublic || hasBypassParam) {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    // For non-public paths, wait for session to be loaded
    if (!isPublicPath) {
      console.log(`Auth loading for protected path: ${currentPath}. Status: ${status}`);
      
      // Use a reasonable timeout to ensure session is properly hydrated
  ...
  }, [status, isPublicPath, currentPath]);

  // Track loading time and show error after timeout for protected paths
  useEffect(() => {
    // Only start the timer if we're loading and on a protected path
    if ((isLoading || status === 'loading') && !isPublicPath) {
        ...
        setLoadingTime(prev => {
          ...
          if (newTime === 3) {
            ...
            if (status === 'unauthenticated' || status === 'loading') {
              window.location.href = `/sign-in?auth_timeout=true&callbackUrl=${encodeURIComponent(currentPath)}`;
            }
          }
          return newTime;
        });
  ...
  }, [isLoading, status, currentPath, isPublicPath]);

  // If still loading but we've reached the timeout, show a helpful error screen
  if ((isLoading || status === 'loading') && showError && !isPublicPath) {
    return (
  ...
  // Regular loading screen for protected paths, will show for up to 3 seconds before error
  if ((isLoading || status === 'loading') && !isPublicPath) {
    return (
  ...
  // For public paths, simply render the children
  if (isPublicPath) {
    return <>{children}</>;
  }
