...
export default function ApplicationsPage({ params }: { params: { callId: string } }) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const castingCallId = params.callId;
  ...
  const handleUpdateStatus = async (applicationId: string, newStatus: string, addToProject: boolean = false) => {
    try {
      ...
      if (newStatus === 'APPROVED' && (castingCall?.project || castingCall?.Project) && addToProject) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await response.json().catch(() => {
        console.warn('Could not parse response, but continuing since status was OK');
      });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-3">
          ...
          {(castingCall?.project || castingCall?.Project) && (
            <Link
              href={`/studio/projects/${(castingCall.project || castingCall.Project)?.id}/casting`}
              className="px-4 py-2 text-blue-600 border border-blue-600 rounded hover:bg-blue-50"
            >
                                    ...
                                    <div className="mt-4">
                                      <div className="flex items-center mb-2">
                                        ...
                                        <label htmlFor="addToProject" className="ml-2 text-gray-900">
                                          Add to project: {(castingCall.project || castingCall.Project)?.title}
                                        </label>
                                      </div>
